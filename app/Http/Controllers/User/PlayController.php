...
class PlayController extends Controller 
{
    ...
    public function playHeadTail($game, $request) 
    {
        ...
        //Log:info('The random value is: ' . $random);
        $res['game_id'] = $invest['game_log']->id;
        $res['balance'] = $user->balance;
        
        Log:info('The random value is: ' . $random. 'The res value is: ' . response()->json($res));
    ...
    public function gameEndHeadTail($game, $request) {
        ...
        Log:info('gameendHeadTail response'.response()->json($res));
        return response()->json($res);
    }
    ...
    public function fallback($request, $user, $game) 
    {
        ...
        if ($running) {
            return ['error' => '1 game is in-complete. Please wait'. 'If the problem persist please contact admin'];
        }
