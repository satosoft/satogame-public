...
class Captcha {
    ...
    public static function verifyCustomCaptcha() {
        ...
        Log::info('Sato Request data is :', ['data' => request()->all()]);

        if ($customCaptcha) {
            $UserEnterCaptchaHashValue = hash_hmac('sha256', request()->captcha, $customCaptcha->shortcode->random_key->value);
            if ($UserEnterCaptchaHashValue != request()->captcha_secret) {
                $pass = false;
            }
        }
        return $pass;
